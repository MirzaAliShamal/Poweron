...
@section('content')
    <div class="card">
        ...
        <div class="card-body py-10">
            <form action="{{ route('subscribers.update', $subscriber->id) }}" method="POST" class="edit-form">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-12">
                        <div class="form-group mb-5">
                            ...
                            <input type="text" name="name" class="form-control" placeholder="John Doe" value="{{ old('name', $subscriber->name) }}"/>
                        </div>
                    </div>
                    <div class="col-lg-12 col-md-12 col-12">
                        <div class="form-group mb-5">
                            <label class="required form-label">Email</label>
                            <input type="email" name="email" class="form-control" placeholder="subscriber@example.com" value="{{ old('email', $subscriber->email) }}"/>
                        </div>
                    </div>
                    <div class="col-lg-12 col-md-12 col-12">
                        <div class="form-group mb-5">
                            ...
                            <select name="email_list_id" class="form-select" data-control="select2" data-placeholder="Choose Email List">
                                ...
                                @foreach ($emailLists as $el)
                                    <option value="{{ $el->id }}" {{ old('email_list_id', $subscriber->email_list_id) == $el->id ? 'selected' : '' }}>{{ $el->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
@section('js')
    ...
    <script src="{{ asset('assets/js/subscribers/edit.js?v='.rand()) }}"></script>
@endsection
